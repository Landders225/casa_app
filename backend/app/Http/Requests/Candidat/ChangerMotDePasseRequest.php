 ...
 *
 *   PUT /api/candidat/mot-de-passe
 *   { current_password, password, password_confirmation }
 *
 * `current_password` : preuve de connaissance du mot de passe ACTUEL — JAMAIS
 * de changement sans elle (vérifiée manuellement via `Hash::check` contre
 * `mot_de_passe_hash`, pas la règle `current_password` de Laravel qui suppose
 * une colonne `password` sur le guard par défaut).
 *
 * `password` : mêmes règles qu'à l'inscription (ADR-16), + `different` du mot
 * de passe actuel (un « changement » identique n'en est pas un).
 */

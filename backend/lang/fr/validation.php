...
/**
 * Traductions françaises des messages de validation Laravel — SOURCE UNIQUE
 * du fallback (bug trouvé en conditions réelles : sans ce fichier, TOUTE
 * règle de validation sans message personnalisé explicite dans un
 * FormRequest affichait sa clé technique brute, ex. « validation.required »,
 * au lieu d'un texte lisible — pas seulement `Password::min()`).
 *
 * Laravel 12 ne fournit plus de fichiers de langue par défaut (retirés du
 * squelette depuis Laravel 9) : sans ce fichier ET sans message personnalisé
 * pour une règle donnée, le traducteur affiche la clé technique telle quelle.
 *
 * Les messages personnalisés déjà écrits dans chaque `FormRequest::messages()`
 * (ex. `'email.unique' => 'Un compte existe déjà...'`) restent PRIORITAIRES —
 * ce fichier ne les remplace pas, il ne comble que ce qui n'a AUCUN message
 * personnalisé. `APP_LOCALE=fr` ET `APP_FALLBACK_LOCALE=fr` (backend/.env) :
 * une seule locale à couvrir, pas de version anglaise nécessaire.
 *
 * Traduction fidèle et complète de la structure de référence Laravel
 * (`vendor/laravel/framework/.../Translation/lang/en/validation.php`) — TOUTES
 * les clés, pas seulement celles utilisées aujourd'hui : une règle ajoutée
 * demain (ex. `alpha`, `url`) reste couverte sans y repenser.
 */
return [
    ...
    'password' => [
        ...
        'uncompromised' => 'Ce :attribute est trop répandu ou a déjà fuité dans une base de données compromises. Choisissez-en un autre.',
    ],
    ...
    |--------------------------------------------------------------------------
    |
    | Aucun ici par convention CASA : chaque FormRequest porte ses propres
    | messages personnalisés (convention `champ.règle`) dans sa méthode
    | `messages()` — c'est CETTE voie-là qui prend le pas sur ce fichier.
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Pour que les messages ci-dessus restent lisibles sur les champs SANS
    | message personnalisé (ex. « Le champ mot de passe est obligatoire »
    | plutôt que « Le champ password est obligatoire »). Liste dressée à
    | partir de tous les champs réellement validés dans `app/Http/Requests`.
    */
    ...
    'attributes' => [
        ...
        'residence_ci' => 'résidence en Côte d\'Ivoire',
        'cgu' => 'conditions d\'utilisation',
        'nationalite' => 'nationalité',
        'nationalite_confirmee' => 'nationalité confirmée',
        'diplome' => 'diplôme',
        'diplome_verifie' => 'diplôme vérifié',
        'sc04' => 'plus haut diplôme',
        'sc04_plus_haut_diplome' => 'plus haut diplôme',
        'role' => 'rôle',
        'poste' => 'poste',
        'actif' => 'statut actif',
        'id' => 'identifiant',
        ...
        'filiere_id' => 'filière',
        'motif' => 'motif',
        'motif_interne' => 'motif interne',
        'motif_communicable' => 'motif communicable',
        'candidature_id' => 'candidature',
        'candidature_ids' => 'candidatures',
        ...
        'commentaire_evaluateur' => 'commentaire de l\'évaluateur',
        'mo04_note_etoiles' => 'note en étoiles',
        'domaine' => 'domaine',
        'duree_categorie' => 'durée',
        'ordre' => 'ordre',
        'fichier' => 'fichier',
    ],

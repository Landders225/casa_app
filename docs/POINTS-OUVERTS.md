...
>
> Dernière revue : **Lot 15a** (2026-09-11) — finitions self-service équipe :
> mot de passe personnel (`PUT /api/equipe/mot-de-passe`, écran « Mon compte »
> partagé évaluateur/admin) et édition d'identité par l'admin (prénom/nom/
> poste, `PATCH /api/admin/membres/{u}` étendu). Backend dupliqué / frontend
> partagé selon la couche (Étape 1, Q1). Avant : **Lot 14** (écran candidat
> « Documents », re-consultation/téléchargement en lecture seule
> post-soumission — bug `rel="noreferrer"` cassant le téléchargement (401),
> corrigé ici et côté évaluateur `FicheCandidat.jsx`) ; **revue espace
...
| **Verrouiller l'identité candidat après soumission de la candidature** | 🟢 **Traité (Lot 15b)** | `PATCH /candidat/profil` refuse désormais (`prohibited`, 422) `prenom`/`nom`/`sexe`/`date_naissance`/`cni` dès que la candidature la plus récente a `date_soumission` non nul — exactement l'« identité stable » vérifiée sur pièce par l'évaluateur (ADR-07), qui la lit toujours **en direct**. `telephone`/`ville_residence` restent éditables dans tous les cas. Signal choisi : `date_soumission` (pas `statut_interne`) — déjà ce que lit le frontend, pas une liste de statuts à maintenir en double. Verrouillage RÉEL côté frontend (champs `disabled`, payload qui les EXCLUT — pas seulement un bandeau) | **ADR-32** (addendum Lot 15b) ; `MettreAJourProfilRequest` ; `Profil.jsx` ; `ProfilCandidatTest` | — |
...
| **Changement de mot de passe équipe** (évaluateur / admin, self-service connecté) | 🟢 **Traité (Lot 15a)** | `PUT /api/equipe/mot-de-passe`, même contrat que le Lot 13 candidat (mot de passe actuel exigé, invalide les AUTRES sessions, e-mail de confirmation) — testé pour évaluateur ET admin. Backend DUPLIQUÉ (`Equipe\MotDePasseController`, même précédent que `Auth\`/`Candidat\MotDePasseController`) ; frontend PARTAGÉ (`ChangementMotDePasseCard` déplacé en composant commun, prop `endpoint`) — choix asymétrique justifié par couche (Étape 1, Q1). Accessible depuis le nouvel écran « Mon compte » (`/equipe/mon-compte`) | **ADR-32** (Q5) ; `Equipe\ChangerMotDePasseRequest` ; `components/account/ChangementMotDePasseCard.jsx` ; `pages/equipe/MonCompte.jsx` ; `Equipe/ChangementMotDePasseTest` ; E2E `mon-compte.spec.js` | — |
...
| **Paramètres — Sécurité & Système** (onglets maquette `parametres.html`) | 🟢 Sécurité **traité (Lot 15a, via « Mon compte »)** / 🟠 Système confort | **Sécurité** : la fonction (changer son propre mot de passe) est livrée depuis le Lot 15a — via l'écran dédié « Mon compte », pas un onglet de `parametres.html` (cet écran-tabulé-là lui-même n'est pas construit, seule la fonction l'est, par un autre point d'entrée UI). **Système** = valeurs de supervision en lecture seule (dernière sauvegarde, version, statut des services) — reste ouvert, faible enjeu | ADR-32 (Q5) ; voir « Changement de mot de passe équipe » ci-dessus ; maquette `parametres.html` (onglets « Sécurité »/« Système ») | Rien pour Sécurité (déjà couvert) ; éventuel endpoint healthcheck simple pour Système |
...
| **Édition de l'identité d'un membre** (corriger prénom / nom / poste d'un compte d'équipe) | 🟢 **Traité (Lot 15a)** | Même route `PATCH /api/admin/membres/{u}` (Lot 11b) étendue — `{prenom,nom,poste}` les 3 ensemble, en plus de `{actif}` déjà existant, jamais combinés par l'UI (2 actions distinctes). Admin STRICT (un évaluateur n'atteint pas le contrôleur, même sur sa propre fiche) ; `role`/`email`/`password`/`mot_de_passe_hash` restent `prohibited` — vérifié explicitement, y compris glissés dans le même payload que l'identité. 1 ligne d'audit (« Modification d'identité ») par appel | **ADR-29** (extension) ; `ModifierMembreRequest` (ex-`ActiverMembreRequest`) ; `GestionCompteEquipe::modifierIdentite` ; `ModifierIdentiteModal.jsx` ; `GestionMembresEquipeTest` ; E2E `equipe.spec.js` ; maquette `utilisateurs.html` (bouton « Gérer ») | — |
...
| **Export Excel mis en page** des rapports | 🟢 **Traité (Lot 15c)** | `RapportExcel` (PhpSpreadsheet) sérialise en `.xlsx` MIS EN FORME (titres, en-têtes en gras, colonnes dimensionnées) le MÊME résultat agrégé que le CSV — aucune 2ᵉ logique de calcul ni de masquage, `ServiceRapports` reste la source unique. Garde-fou k-anonymat prouvé en RELISANT le classeur généré (PhpSpreadsheet en lecture dans les tests), pas seulement en inspectant le code qui l'écrit | **ADR-30** (addendum Lot 15c) ; `RapportController::exportXlsx` ; `Rapports.jsx` (bouton Excel activé) ; `RapportsStatistiquesTest` | — |
| **Export PDF mis en page** des rapports | 🟠 Ouvert — **décidé hors périmètre pour l'instant** | Le CSV couvre la donnée brute, l'Excel (ci-dessus) la présentation exploitable (tri/filtres) — un PDF répond à un besoin différent (document figé/officiel) qui n'était pas confirmé au Lot 15c. Bouton affiché désactivé « à venir » | **ADR-30** (D-11c-4) ; `Rapports.jsx` (bouton PDF `disabled`) | Lib (DomPDF) + gabarit de document + le même passage par `ServiceRapports` — **à ouvrir seulement si un besoin réel de document figé/officiel se confirme** |
...
| **Ré-notification si l'entretien est replanifié** (date/heure/lieu modifiés après la première planification) | 🟢 **Traité (Lot 15b)** | Jusqu'ici, seule la PREMIÈRE planification envoyait un mail (`EntretienPlanifie`) — une replanification ultérieure ne prévenait pas le candidat (point ouvert signalé dans le code depuis le Lot 12b, ADR-33 §572). Nouvelle classe dédiée `EntretienReplanifie` (même patron 1-classe-par-événement que l'ADR-33, canaux `mail`+`database`), déclenchée UNIQUEMENT si date/heure/lieu changent réellement (comparaison normalisée — jamais sur une simple mise à jour presence/observation/notes) sur un entretien qui existait déjà. Bouton « Modifier la planification » ajouté dans `Entretien.jsx` (réutilise le formulaire de création, pré-rempli) tant que l'entretien n'est pas verrouillé — sans lui, le mécanisme serveur aurait été inatteignable en pratique | **ADR-33** (addendum Lot 15b) ; `App\Notifications\EntretienReplanifie` ; `EntretienController::update` ; `EntretienReplanifieNotificationTest` ; `Entretien.jsx` (composant `Planification` réutilisé) ; E2E `notation.spec.js` | — |
...
| **Historique in-app des notifications — évaluateur/admin** | 🟠 Ouvert — **vérifié au Lot 15c : rien à afficher aujourd'hui** | Inventaire exhaustif des `->notify()`/`Notification::send()` du backend (Lot 15c) : les 5 notifications `database` existantes (`InscriptionConfirmee`, `CandidatureSoumise`, `EntretienPlanifie`, `EntretienReplanifie`, `ResultatsPublies`) vont **toutes** au candidat seul ; `MotDePasseModifie` atteint bien un compte équipe (`Equipe\MotDePasseController`) mais son `via()` ne renvoie que `['mail']`, jamais `database` ; `AffectationController` n'appelle `notify()` nulle part. Construire l'écran maintenant donnerait une façade **vide en permanence** — confirmé, pas construit | Kickoff Lot 12c (périmètre candidat) ; inventaire Lot 15c | **Ne pas construire l'écran avant d'avoir choisi un premier événement.** Piste prioritaire : **affectation d'un dossier à un évaluateur** (`AffectationController`) — signal clair (« vous avez du travail »), événement déjà centralisé en un seul point d'entrée, patron identique à `CandidatureSoumise` (Lot 12b). Piste secondaire, valeur ajoutée plus faible : notifier l'évaluateur d'une correction exceptionnelle sur SON dossier (déjà visible immédiatement dans l'UI + tracé à l'audit) |

...
cp .env.production.example .env.production                 # vars compose : POSTGRES_PASSWORD (fort), TLS_DIR
cp backend/.env.production.example backend/.env.production  # Laravel : voir le tableau ci-dessous
docker compose --env-file .env.production \
  -f docker-compose.yml -f docker-compose.prod.yml run --rm backend php artisan key:generate --show
#   → coller la clé dans APP_KEY de backend/.env.production

# 2. Certificat de BOOTSTRAP (nginx exige un cert pour démarrer ; Let's Encrypt
...
sudo certbot certonly --webroot -w ./docker/nginx/acme-webroot -d casa.example.org \
  --deploy-hook "DOMAIN=casa.example.org TLS_DIR=$(pwd)/docker/nginx/tls COMPOSE_DIR=$(pwd) $(pwd)/docker/nginx/renew-hook.sh"
# Le renouvellement est ensuite automatique (timer systemd de certbot).
# Variante conteneurisée (service `certbot` dans compose) : possible, non retenue (ADR-27).
```

### Tableau : variable → dev → prod → à changer

| Variable (`backend/.env.production`) | Dev (`localhost`) | Prod (`casa.example.org`) | Action avant déploiement |
|---|---|---|---|
| `APP_ENV` | `local` | `production` | — (fixé dans l'`.example`) |
| `APP_DEBUG` | `true` | `false` | — (fixé) |
| `APP_KEY` | *(clé dev)* | *(clé DÉDIÉE)* | **`key:generate --show` → coller** |
| `APP_URL` | `http://localhost:8080` | `https://casa.example.org` | **domaine, avec `https://`, sans `/` final** |
| `SESSION_DOMAIN` | `localhost` | `casa.example.org` | **host exact, sans port/schéma** (`.example.org` si apex+www) |
| `SANCTUM_STATEFUL_DOMAINS` | `localhost:8080,127.0.0.1:8080` | `casa.example.org` | **host(s) ; port seulement si non-standard** |
| `SESSION_SECURE_COOKIE` | *(absent → false)* | `true` | — (fixé) |
| `SESSION_SAME_SITE` | `lax` | `lax` | — (ni `strict`, ni `none` : cf. `.example`) |
| `DB_PASSWORD` = `POSTGRES_PASSWORD` (racine) | `casa` | *(fort)* | **`openssl rand -base64 24`, identique des deux côtés** |
| `TRUSTED_PROXIES` | *(absent → `*`)* | `*` | **recommandé : resserrer au sous-réseau Docker** (`docker network inspect …`) |
| `LOG_CHANNEL` / `LOG_LEVEL` | `stack` / `debug` | `stderr` / `warning` | — (fixé) |
| `VITE_API_URL` (front) | `/api` | `/api` | **inchangé — relatif, le build est domaine-agnostique** |
| `DOMAIN` (script gen-selfsigned) | `casa.localhost` | `casa.example.org` | **domaine** |
| `TLS_DIR` (`.env.production` racine) | `./docker/nginx/tls` | `./docker/nginx/tls` | — (alimenté par `renew-hook.sh`) |

**En-têtes servis** (bloc HTTPS, `add_header … always`) : `Strict-Transport-Security` (1 an, `includeSubDomains`), `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (camera/micro/géo/paiement désactivés), **CSP** stricte : `script-src 'self'` (aucun script inline dans le build), seuls relâchements `style-src 'unsafe-inline'` (attributs `style={{…}}` React) et `font-src data:` (sous-jeux de police < 4 Ko inlinés par Vite) — aucun vecteur d'exécution de code. `http://` → **301** `https://` (sauf `/.well-known/acme-challenge/`).
...
# E2E — parcours d'intégration bout-en-bout (~15 min, avant déploiement / en CI)
#   base JETABLE : migrate:fresh --seed avant/après ; publication irréversible
